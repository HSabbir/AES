import math
import tkinter as tk
from tkinter import filedialog
#root = tk.Tk()


file1=tk.filedialog.askopenfilename()
file2=tk.filedialog.askopenfilename()
#file1 = 'key.txt'
#file2 = 'data.txt'
key = [[0 for x in range(4)] for y in range(4)]


arr = [[2, 3, 1, 1], [1, 2, 3, 1], [1, 1, 2, 3], [3, 1, 1, 2]]
inv_arr = [[14, 11, 13, 9], [9, 14, 11, 13], [13, 9, 14, 11], [11, 13, 9, 14]]

s_box = [
    ,
]


inverse_s_box = [
    ,
]

Rcon = [
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36,
]

def initializeKey():
    with open(file1, 'r+') as f:
        text = f.read()

    f.close()
    # print(text)
    l = len(text)
    for i in range(l, 16):
        text += '0'

    l = 0
    for i in range(4):
        for j in range(4):
            key[j][i] = ord(text[l])
            l += 1

def substituteBytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]

    return s

def inverse_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = inverse_s_box[s[i][j]]

    return s

def shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]

    return s

def inverse_shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

    return s

def add_round_key(s, k, round):
    # print('k : ',k)
    for i in range(4):
        for j in range(4):
            s[j][i] ^= k[j][(round *4) + i]

    return s

def rotword(word):
    return word[1:] + word[:1]

def substituteWord(word):

    for i in range(4):
        word[i] = s_box[word[i]]
        # print(word)

    return word



def productBy2(ch):
    if ch >> 7:
        return ((ch << 1) ^ 27) % 256
    else:
        return (ch << 1) % 256


# print(productBy2(0x87))
# print(24<<1)

def numberProduct(n, ch):
    if n == 0:
        return ch
    else:
        for i in range(n):
            ch = productBy2(ch)

        return ch

def product(x, ch):
    temp = 0
    while x >= 1:
        power = int(math.log2(x))
        temp ^= numberProduct(power, ch)
        x = x % int(2 ** power)

    return temp


# print(math.log2(1))


def mixColumn(state):
    temp = [[0 for x in range(4)] for y in range(4)]

    for i in range(4):
        for j in range(4):
            for k in range(4):
                temp[j][i] ^= product(arr[j][k], state[k][i])

    return temp

def inverse_mixColumn(state):
    temp = [[0 for x in range(4)] for y in range(4)]

    for i in range(4):
        for j in range(4):
            for k in range(4):
                temp[j][i] ^= product(inv_arr[j][k], state[k][i])

    return temp



def getMsg(pt):
    s = ''
    # print(pt)
    for i in range(4):
        for j in range(4):
            s += chr(pt[j][i])

    return s

def encrypt(pt, expandKey):
    pt = add_round_key(pt, expandKey, 0)

    for r in range(1, 11):
        pt = substituteBytes(pt)
        pt = shift_rows(pt)
        if r < 10:
            pt = mixColumn(pt)
        pt = add_round_key(pt, expandKey, r)

    return pt

def decrypt(pt, expandKey):
    pt = add_round_key(pt, expandKey, 10)

    for i in range(9, -1, -1):
        pt = inverse_shift_rows(pt)
        pt = inverse_sub_bytes(pt)
        pt = add_round_key(pt, expandKey, i)
        if i > 0:
            pt = inverse_mixColumn(pt)


    return pt



def KeyExpansion():
    w = [[0 for x in range(44)] for y in range(4)]

    for i in range(4):
        for j in range(4):
            w[j][i] = key[j][i]

    temp = [0] * 4
    for i in range(4, 44):
        for j in range(4):
            temp[j] = w[j][i - 1]

        if i % 4 == 0:
            temp = substituteWord(rotword(temp))
            temp[0] ^= Rcon[i // 4]

        for j in range(4):
            w[j][i] = temp[j] ^ w[j][i - 4]


    return w


#************************cipher block chaining****************************
def cbc(data, block, y):
    cipher = ''
    orginal = ''
    out1 = open("cbc.txt", "w+", encoding='ascii', errors='ignore')
    out2 = open("cipher_cbc.txt", 'w+', encoding='ascii', errors='ignore')
    temp = [[0 for x in range(4)] for y in range(4)]

    for p in range(block):
        pt = [[0 for x in range(4)] for y in range(4)]
        l = 0
        subData = data[p * 16: (p + 1) * 16]
        # print(subData)

        for i in range(4):
            for j in range(4):
                pt[j][i] = ord(subData[l])
                pt[j][i] ^= temp[j][i]
                l += 1


        # print(pt)
        dec = [[0 for x in range(4)] for y in range(4)]
        pt = encrypt(pt, expandKey)
        cipher += getMsg(pt)

        for i in range(4):
            for j in range(4):
                dec[j][i] = temp[j][i]
                temp[j][i] = pt[j][i]

        pt = decrypt(pt, expandKey)
        for i in range(4):
            for j in range(4):

                pt[j][i] ^= dec[j][i]

        orginal += getMsg(pt)


    print('\nCBC chiper text : \n', cipher)
    print(type(orginal))
    print('\norginal text : \n', orginal[:-(16 - y)])
    out1.write(orginal[:-(16 - y)])
    out1.close()

    out2.write(str(cipher.encode('ascii', 'ignore')))
    out2.close()



#********************************electronic codebook mode***********************************
def ecb(data, block, y):
    cipher = ''
    orginal = ''
    out1 = open("ecb.txt", "w+", encoding='ascii', errors='ignore')
    out2 = open("cipher_ecb.txt", 'w+', encoding='ascii', errors='ignore')

    for p in range(block):
        pt = [[0 for x in range(4)] for y in range(4)]
        l = 0
        subData = data[p * 16: (p + 1) * 16]
        # print(subData)

        for i in range(4):
            for j in range(4):
                pt[j][i] = ord(subData[l])
                l += 1

        # print(pt)
        pt = encrypt(pt, expandKey)
        cipher += getMsg(pt)

        pt = decrypt(pt, expandKey)
        orginal += getMsg(pt)

    print('EBC chiper text : \n', cipher)
    print('orginal text : \n', orginal[:-(16 - y)])
    out1.write(orginal[:-(16 - y)])
    out1.close()

    out2.write(str(cipher.encode('ascii', 'ignore')))
    out2.close()



#***************************main function starts here***************************
initializeKey()
expandKey = KeyExpansion()

data = ''

fh = open(file2, encoding='ascii', errors='ignore')
data += fh.read()
fh.close()


y = len(data) % 16
if y:
    for i in range(16 -y):
        data += '0'

block = len(data) // 16
print(block)

ecb(data, block, y)
cbc(data, block, y)